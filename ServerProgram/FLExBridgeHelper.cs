...
namespace ServerProgram
{
	...
	public class FLExBridgeHelper
	{
		...
		private const string FLExBridgeName = @"ClientProgram.exe";
		...
		private static void LaunchFlexBridge(IIPCHost host, string command, string args, ref bool changesReceived, ref string projectName)
		{
			...
			{
				Console.WriteLine("FLExBridgeHelper.LaunchFlexBridge(): not waiting for anything");
			...
			else
			{
				...
				Console.WriteLine("FLExBridgeHelper.LaunchFlexBridge(): done waiting for FlexBridge to terminate (flag = {0})", _flexBridgeTerminated);
		...
		{
			Console.WriteLine("executing FLExBridgeHelper.KillTheHost()");
		...
		public static string FixItAppPathname
		{
			get
			{
				var tmp = FieldWorksAppsDir;
				var prog = "ClientProgram.exe";
				return Path.Combine(tmp, prog);
		...
		public static string FieldWorksAppsDir
		{
			get
			{
				string tmp = Path.GetDirectoryName(FileUtils.StripFilePrefix(Assembly.GetExecutingAssembly().CodeBase));
				return tmp;
			}
		}
		...
		public class FLExBridgeService : IFLExBridgeService
		{
		...
		{
			Console.WriteLine("FLExBridgeHelper.AlertFlex()");
		...
		/// </summary>
		/// <remarks>
		/// This interface combines the service and channel objects so a factory can give us a useful object
		/// </remarks>
		[ServiceContract]
		public interface IFLExService : IClientChannel
		{
